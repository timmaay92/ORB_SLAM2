...
void SubscribeHandler::Publish_Orientation(cv::Mat Twc, tf::StampedTransform T_w_c)
{

    Eigen::Matrix<double, 3, 3> Twc_eig = SubscribeHandler::toMatrix3d(Twc.clone());
    std::vector<float> q = SubscribeHandler::toQuaternion(Twc_eig);

    // TF fill broadcast message
    tf::Transform TForientation;

    TForientation.setOrigin(tf::Vector3(Twc.at<float>(0,3), Twc.at<float>(1,3),Twc.at<float>(2,3)));
    ...
    orientation_msg.pose.position.x = Twc.at<float>(0,3);
    orientation_msg.pose.position.y = Twc.at<float>(1,3);
    orientation_msg.pose.position.z = Twc.at<float>(2,3);
